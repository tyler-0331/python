...
data['rain_yn'] = (data.loc[:,('sumRn')] > 0 ) * 1    #비옴 : 1, 비 안옴: 0
print(data.head(3))
...
print(sp[:3])  # [    0     0]  [18000     1]

tg1 = sp[sp[:,1] == 0, 0]    # 비 안올 때 매출액
tg2 = sp[sp[:,1] == 1, 0]    # 비 안올 때 매출액
print(tg1[:3])
print(tg2[:3])
...
# 두 집단의 각 평균
print('비 안올 때 매출액: ',np.mean(tg1),' 비 올 때 매출액: ',np.mean(tg2))  #761040.2542372881   757331.5217391305
...
print(stats.shapiro(tg1).pvalue)   # 0.0560494  > 0.05  그러므로 만족
print(stats.shapiro(tg2).pvalue)   # 0.8827397  > 0.05  그러므로 만족
