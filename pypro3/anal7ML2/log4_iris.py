...
# 독립변수를 스케일링을 하면 모델이 안정성, 수렴 속도 향상, 오버플로우/언더플로우 등의 방지에 효과적
print(x_train[:3]) 
...
# model 
model = LogisticRegression(C = 100.0, random_state = 0, solver = 'lbfgs', multi_class ='auto')  # C 속성: L2 규제  (over-fitting 방지하기 위함)
# C 속성: L2 규제 값이 작을수록 더 강한 정규화 규제가 진행됨
print(model)
...
print('총 겟수:%d, 오류수:%d'%(len(y_test), (y_test != y_pred).sum()))
